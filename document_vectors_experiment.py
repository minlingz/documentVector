""" Author: Meixiang Du(md480), Minling Zhou(mz246)"""
...
# print(f"Austen sentences: {len(austen)}")
carroll = nltk.corpus.gutenberg.sents("carroll-alice.txt")
# print(f"Carroll sentences: {len(carroll)}")
...
def generate_data_word2vec(
    ...
    embeddings = []
    for document in h0_documents + h1_documents:
    ...
    y: FloatArray = np.array([0 for _ in h0_documents] + [1 for _ in h1_documents])
    return split_train_test(X, y)
